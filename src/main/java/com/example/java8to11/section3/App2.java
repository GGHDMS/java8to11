...
public class App2 {
    ...
    public static void main(String[] args) {
        ...
        javaClasses.add(new OnlineClass(7, "The Java, Code maniploation", false));
        javaClasses.add(new OnlineClass(8, "The Java, 8 to 11", false));

        List<List<OnlineClass>> seokmunEvent = new ArrayList<>();
        seokmunEvent.add(springClasses);
        seokmunEvent.add(javaClasses);

        System.out.println("두 수업 목록에 들어있는 모든 수업 아이디 출력");
        seokmunEvent.stream().flatMap(Collection::stream).forEach(c -> System.out.println(c.getId()));

        System.out.println("10부터 1씩 증가하는 무제한 스트림 중에서 앞에 10개 빼고 최대 10개 까지만");
        Stream<Integer> iterate = Stream.iterate(10, i -> i + 1);// 중개형
